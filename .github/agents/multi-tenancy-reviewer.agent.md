...
- **Before reviewing**: `search_thoughts("tenant isolation findings", project: "Falsify", created_by: "copilot-vscode", type: "bug")` — loads prior data leakage risks and RLS patterns
- **After review**: `capture_thought("Multi-Tenancy Reviewer: <N findings — key issues>", project: "Falsify", created_by: "copilot-vscode", source: "agent-multi-tenancy-reviewer")` — persists tenant isolation gaps and remediation patterns
