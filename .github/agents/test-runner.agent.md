...
- **Before running tests**: `search_thoughts("test failures", project: "Falsify", created_by: "copilot-vscode", type: "bug")` — load known flaky tests, prior failure patterns, and test infrastructure issues
- **After test run**: `capture_thought("Test run: <N passed, N failed — key failure patterns>", project: "Falsify", created_by: "copilot-vscode", source: "agent-test-runner")` — persist test outcomes
