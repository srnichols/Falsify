...
- [ ] All prerequisites verified — `node --version`, `npm --version` returns expected versions
- [ ] `npm install && npm run build` succeeds without errors
- [ ] `npm test` passes on the new environment
...
- **Before onboarding**: `search_thoughts("onboarding", project: "Falsify", created_by: "copilot-vscode", type: "convention")` — load known setup issues and environment quirks
- **After onboarding**: `capture_thought("Onboarding: <environment status, blockers encountered>", project: "Falsify", created_by: "copilot-vscode", source: "skill-onboarding")` — persist setup issues for future new developers
