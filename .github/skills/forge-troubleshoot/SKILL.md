...
- Unresolved `Falsify` or `TypeScript / React / Node.js` placeholders
...
- **Before diagnosing**: `search_thoughts("forge failure", project: "Falsify", created_by: "copilot-vscode", type: "bug")` — load prior forge failures and recurring issues to avoid repeating diagnoses
- **After diagnosis**: `capture_thought("Forge troubleshoot: <symptom> → <root cause> → <fix>", project: "Falsify", created_by: "copilot-vscode", source: "skill-forge-troubleshoot")` — persist findings for future diagnostics
