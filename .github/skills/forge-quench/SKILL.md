---
name: forge-quench
description: "Systematically reduce TypeScript code complexity while preserving exact behavior — measure, understand, propose, prove, report. Use after a feature is complete and tests pass, when code works but is harder to maintain than it should be."
argument-hint: "[optional: specific files or directories to simplify, e.g. 'src/services/' or 'src/services/userService.ts']"
tools:
  - run_in_terminal
  - read_file
  - grep_search
...
Identify the most complex functions in the target files. Use TypeScript-appropriate static analysis:

```bash
# Cyclomatic complexity via ESLint
npx eslint --rule 'complexity: [error, 10]' <target>

# Dedicated complexity analysis (if available)
npx ts-complexity <target>

# Count functions exceeding 50 lines
grep -rn "function\|=>\|async " <target> --include="*.ts" --include="*.tsx" | head -30

# Manual analysis: identify deep nesting, long switch statements, excessive branching
grep -rn "if\|else\|switch\|case\|catch" <target> --include="*.ts" --include="*.tsx" | wc -l
```
...
2. **Run the full test suite** immediately (`pnpm test` or `npm test`)
3. **If tests pass**: Commit with a descriptive message:
   ...
   Behavior: unchanged — all N tests pass (`pnpm test`)
   ```
...
- **Before simplifying**: `search_thoughts("code complexity", project: "Falsify", created_by: "copilot-vscode", type: "pattern")` — load prior simplification decisions, functions intentionally left complex, and patterns that worked
- **After simplifying**: `capture_thought("Forge Quench: <N functions simplified, N skipped — key changes>", project: "Falsify", created_by: "copilot-vscode", source: "skill-forge-quench")` — persist what was simplified and what was intentionally left complex for future sessions
