...
- [ ] All suites executed — `npm test` completes
- [ ] Zero unexplained failures (every failure has a documented reason)
- [ ] Coverage report generated — `npx jest --coverage` or `npx vitest --coverage`
- [ ] Coverage not decreased from baseline
- [ ] `forge_sweep` found zero production code markers (TODO/FIXME/stub)
## Persistent Memory (if OpenBrain is configured)

- **Before running tests**: `search_thoughts("test failures", project: "Falsify", created_by: "copilot-vscode", type: "bug")` — load known flaky tests, recurring failures, and environment-specific issues
- **After test sweep**: `capture_thought("Test sweep: <N passed, N failed — key failure patterns>", project: "Falsify", created_by: "copilot-vscode", source: "skill-test-sweep")` — persist failure patterns and flaky test discoveries
