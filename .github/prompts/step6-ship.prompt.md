...
- **Before shipping**: `search_thoughts("postmortem lessons", project: "Falsify", created_by: "copilot-vscode", type: "postmortem")` — load lessons from prior phase postmortems to check for recurring shipping issues (failed pushes, missed changelog entries, forgotten tags)
- **After postmortem**: `capture_thoughts([...lessons], project: "Falsify", created_by: "copilot-vscode", source: "phase-N-postmortem", type: "postmortem")` — batch capture all lessons, patterns, architecture decisions, bugs, and conventions from this phase
