...
1. **Check OpenBrain** (if configured): `search_thoughts("<FEATURE-NAME>", project: "Falsify")` — load prior decisions, gotchas, and lessons about this feature area. If results found, reference them in the specification.
...
- **Before interviewing**: `search_thoughts("<feature topic>", project: "Falsify", created_by: "copilot-vscode", type: "decision")` — check if this feature or similar has been specified before, load prior decisions and lessons
- **After specification is complete**: `capture_thought("Feature spec: <summary of what and why>", project: "Falsify", created_by: "copilot-vscode", source: "plan-forge-step-0", type: "decision")` — persist the specification for downstream sessions
