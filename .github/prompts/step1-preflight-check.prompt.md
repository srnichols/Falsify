...
- **Check OpenBrain** (if configured): `search_thoughts("<plan topic> blockers", project: "Falsify")` — load prior preflight failures and known blockers
...
- **Before checking**: `search_thoughts("preflight blockers", project: "Falsify", created_by: "copilot-vscode", type: "bug")` — check if prior preflights failed for known reasons
- **If preflight fails**: `capture_thought("Preflight blocker: <what failed and why>", project: "Falsify", created_by: "copilot-vscode", source: "plan-forge-step-1", type: "bug")` — persist the blocker so it's caught earlier next time
- **If preflight passes with notable decisions**: `capture_thought("Preflight passed: <key confirmations — branch strategy, resolved ambiguities>", project: "Falsify", created_by: "copilot-vscode", source: "plan-forge-step-1", type: "decision")` — persist confirmations so next phase doesn't re-ask
